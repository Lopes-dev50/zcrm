<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex flex-row">
            <div class="pr-4 dark:text-gray-400" > @if(isset($cliente))  @else   @endif</div>
            ...
            <div class="pr-4 dark:text-gray-400"><a href="{{route('funil_corretor',['id' =>$enc->encriptar(Auth::user()->id )],'show')}}">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6  border-b bg-white">
                   <!----------inicio------>
                   @if(isset($cliente))
                   <form action="{{ route('add_edit_cliente',['id' =>$enc->encriptar($cliente->id ) ] ,'update') }}" method="post">

                    @method('PUT')
                      @else
                      <form action=" {{ route('add_cliente')}}" method="post">
    ...
    <div class="grid xl:grid-cols-3 xl:gap-6">
        <div class="relative z-0 w-full mb-6 group">
            ...
            <div class="relative z-0 w-full mb-6 group">
                <label for="empreendimento_cliente" class="block mb-2 text-sm font-medium text-gray-400 dark:text-gray-400">Interesse do cliente</label>
                <select id="empreendimento_cliente" name="empreendimento_cliente" class="bg-gray-50 border border-gray-300 text-gray-400 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <option selected>{{$cliente->empreendimento_cliente ?? 'Selecionar'}}</option>
            ...
            <div class="relative z-0 w-full mb-6 group">
                <label for="nascimento_cliente" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Data nascimento</label>
                <input type="date" id="nascimento_cliente" name="nascimento_cliente" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light"  value="{{$cliente->nascimento_cliente ?? ''}}">
                </div>
        </div>
            <div class="grid xl:grid-cols-3 xl:gap-6">
                <div class="relative z-0 w-full mb-6 group">
                    <label for="countries" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Etiqueta do cliente</label>
                    <select id="countries" name="etiqueta" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        <option selected>{{$cliente->etiqueta ?? 'Selecionar'}}</option>
                        <option value="SEM_RENDA">SEM_RENDA</option>
                        <option value="SPC">SPC</option>
                        <option value="REPROVADO">REPROVADO</option>
                        <option value="JA_COMPROU">JÁ_COMPROU</option>
                        <option value="DESISTIU">DESISTIU</option>
                        <option value="SEM_RESPOSTA">SEM_RESPOSTA</option>
                        <option value="PESQUISANDO">PESQUISANDO</option>
                         <option value="INVESTIDOR">INVESTIDOS</option>
...
<br>
                    @can('corretor')
                    @if(isset($cliente))
                    <a href="{{route('deletar_cliente',['id' =>$enc->encriptar($cliente->id )],'destroy')}}" class="inline-block text-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-red-800" viewBox="0 0 20 20" fill="currentColor">
